#!/usr/bin/env python3
# @self-expose: {"id": "fix_api_format_v2", "name": "Fix Api Format V2", "type": "component", "version": "1.0.0", "needs": {"deps": [], "resources": []}, "provides": {"capabilities": ["Fix Api Format V2功能"]}}
# -*- coding: utf-8 -*-
"""
修复前端API列表格式，使其与后端API列表格式一致 - 版本2
"""
...
def fix_frontend_api_list():
    ...
    for api in frontend_apis:
        ...
        # 确保路径以/api开头（除了根路径）
        if not normalized_url.startswith('/') and normalized_url != '/':
            normalized_url = '/api' + normalized_url
        
        fixed_api['path'] = normalized_url
        fixed_apis.append(fixed_api)
    
    # 保存修复后的前端API列表
    with open('frontend_api_list_fixed_v2.json', 'w', encoding='utf-8') as f:
...
def analyze_api_differences():
    ...
    # 读取修复后的前端API列表
    with open('frontend_api_list_fixed_v2.json', 'r', encoding='utf-8') as f:
        frontend_apis = json.load(f)
    ...
    for path in sorted(missing_in_backend):
        # 查找对应的原始URL
        original_urls = [api['url'] for api in frontend_apis if api['path'] == path]
        print(f"  - {path} (来自: {original_urls[0]})")
